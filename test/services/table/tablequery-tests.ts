...
interface GeneratedEntity extends azure.TableService.TableEntity {
  Address: eg.EntityProperty<string>;
  Name: eg.EntityProperty<string>;
}

function generateEntities(count): GeneratedEntity[] {
  var entities = [];
...
describe('tablequery-tests', function () {
  ...
  it('ReplaceOperators', function (done) {
    var TQ = TableQuery as any;
    assert.equal(TQ._replaceOperators(' ==  == '), ' eq  eq ');
    assert.equal(TQ._replaceOperators(' >  > '), ' gt  gt ');
    assert.equal(TQ._replaceOperators(' <  < '), ' lt  lt ');
    assert.equal(TQ._replaceOperators(' >=  >= '), ' ge  ge ');
    assert.equal(TQ._replaceOperators(' <=  <= '), ' le  le ');
    assert.equal(TQ._replaceOperators(' !=  != '), ' ne  ne ');
    assert.equal(TQ._replaceOperators(' &&  && '), ' and  and ');
    assert.equal(TQ._replaceOperators(' ||  || '), ' or  or ');
    assert.equal(TQ._replaceOperators('! !'), 'not not');
      ...
      tableService.createTable(tableName, function (error) {
        ...
        tableService.insertOrMergeEntity(tableName, entity, function (error2) {
          ...
          tableService.insertOrMergeEntity(tableName, entity2, function (error3) {
            ...
            tableService.retrieveEntity<typeof entity>(tableName, entity.PartitionKey['_'], entity.RowKey['_'], function (error4, entityResult) {
    ...
    it('SingleField', function (done) {
      tableService.createTable(tableName, function (createError, table, createResponse) {
        ...
        tableService.executeBatch(tableName, batch, function (batchError, performBatchOperationResponses, batchResponse) {
          ...
          tableService.queryEntities<GeneratedEntity>(tableName, tableQuery, null, function (queryError, queryResult, queryResponse) {
            assert.equal(queryError, null);
            assert.ok(queryResponse.isSuccessful);
            assert.ok(queryResult.entries);
