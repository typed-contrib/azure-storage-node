...
var getNewEntityToTest = function () {
  return {
    ...
    DateTimeProperty: eg.DateTime(new Date(Date.UTC(2014, 4, 7, 08, 20, 53))),
  };
}
...
describe('tableservice-payload-tests', function () {
  ...
  describe('SelectBaseCase', function () {   
    it('FullMetadata', function (done) {
      var options = { payloadFormat: TableUtilities.PayloadFormat.FULL_METADATA };
      fnHelper(selectBaseCaseTest, options, done);
    });

    it('MinimalMetadata', function (done) {
      var options = { payloadFormat: TableUtilities.PayloadFormat.MINIMAL_METADATA };
      fnHelper(selectBaseCaseTest, options, done);
    });

    it('NoMetadata', function (done) {
      var options = { payloadFormat: TableUtilities.PayloadFormat.NO_METADATA };
      fnHelper(selectBaseCaseTest, options, done);
    });
  });
...
function expectPKRKType(options) {
  return {
...
interface ExpectPropTypeResult {
  val: boolean;
  stringOverride?: boolean;
}
function expectPropType(options, property): ExpectPropTypeResult {
  if (options.propertyResolver) {
    return {val: true, stringOverride: false};
  }

  if (property['$'] === 'Edm.Int32' || property['$'] === 'Edm.Double') {
    if ((isNaN(property['_']) || property['_'] === Number.POSITIVE_INFINITY || property['_'] === Number.NEGATIVE_INFINITY) && (options.payloadFormat !== TableUtilities.PayloadFormat.NO_METADATA || options.autoResolveProperties)) {
