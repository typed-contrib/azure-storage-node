...
var fileService: azure.FileService;
...
describe('File', function () {
  ...
  describe('createFile', function () {
    it('should detect incorrect directory names', function (done) {
      assert.throws(function () { fileService.createFile(shareName, directoryName, null, 0, function () { }); },
        function(err) {
          return (err instanceof ArgumentNullError) && err.message === 'Required argument file for function createFile is not defined'; 
        });

      assert.throws(function () { fileService.createFile(shareName, directoryName, '', 0, function () { }); },
        function(err) {
          return (err instanceof ArgumentNullError) && err.message === 'Required argument file for function createFile is not defined'; 
        });
    ...
    it('metadata', function (done) {
      fileService.createFile(shareName, directoryName, fileName, 0, function (createError1, file1, createResponse1) {
        ...
        // creating again is fine
        fileService.createFile(shareName, directoryName, fileName, 5, { contentSettings: properties }, function (createError2, file2, createResponse2) {
          assert.equal(createError2, null);
          assert.notEqual(file2, null);
          assert.equal(createResponse2.statusCode, HttpConstants.HttpResponseCodes.Created);
  ...
  describe('deleteFileIfExists', function() {
    ...
    it('should throw if called without a callback', function (done) {
      assert.throws(function () { (<any>fileService).deleteFileIfExists(shareName, directoryName, fileName); },
        ArgumentNullError
      );
      done();
    });
  ...
  describe('deleteFile', function() {
    ...
    it('should throw if called without a callback', function (done) {
      assert.throws(function () { (<any>fileService).deleteFile(shareName, directoryName, fileName); },
        ArgumentNullError
      );
      done();
    });
  ...
  describe('fileProperties', function () {
    ...
    it('setAndGetAllOnCreate', function (done) {
      fileService.createFile(shareName, directoryName, fileName, 0, { contentSettings: properties }, function (createError) {
        assert.equal(createError, null);
          ...
          fileService.createShare(targetShareName, function (createShareErr) {
            ...
            fileService.createDirectory(targetShareName, targetDirectoryName, function (createDirectoryErr) {
              ...
              fileService.createFileFromText(sourceShareName, sourceDirectoryName, sourceFileName, fileText, options, function (uploadErr) {
                ...
                var destOptions = { metadata : destMetadata };
                fileService.startCopyFile(fileService.getUrl(sourceShareName, sourceDirectoryName, sourceFileName), targetShareName, targetDirectoryName, targetFileName, destOptions, function (copyErr, copyRes) {
