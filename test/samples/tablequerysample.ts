...
function queryAllEntities(callback) {
  // Select all fields
  tableService.queryEntities<EntityType>(tableName, null, null, function (error, result) {
...
function queryEntitiesWhere(callback) {
  ...
  tableService.queryEntities<EntityType>(tableName, tableQuery, null, function (error, result) {
    assert.equal(error, null);
    assert.notEqual(result, null);
    assert.notEqual(result.entries, null);
